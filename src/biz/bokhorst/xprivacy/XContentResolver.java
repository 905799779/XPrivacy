...
public class XContentResolver extends XHook {
		...
		try {
			...
			for (int i = 0; i < count; i++)
				...
				case Cursor.FIELD_TYPE_BLOB:
					...
					break;
				default:
					Util.log(this, Log.WARN, "Unknown cursor data type=" + getType(cursor, i));
				}
	...
	static int getType(Cursor cursor, int index) {
		if (Build.VERSION.SDK_INT > Build.VERSION_CODES.GINGERBREAD_MR1) {
			return getTypeOfCursorHC(cursor, index);
		} else {
			return getTypeOfCursor(cursor, index);
		}
	}
